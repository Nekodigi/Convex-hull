float mapAndConstrain(float a, float b, float c, float d, float e){
  return constrain(map(a, b, c, d, e),d, e);
}

void drawLine(int id1, int id2){
  line(points[id1].x, height-points[id1].y, points[id2].x, height-points[id2].y);
}

void drawPoint(int id){
  drawPoint(id, point_size);
}

void drawPoint(int id, float size){
  ellipse(points[id].x, height-points[id].y, size, size);
}
